package edu.byu.cs.tweeter.client.view.main.signUp;
...
import edu.byu.cs.tweeter.client.presenter.signUp.SignUpPresenter;
import edu.byu.cs.tweeter.client.view.asyncTasks.PostSignUpTask;
...
public class UserSettingsActivity extends AppCompatActivity implements SignUpPresenter.View, PostSignUpTask.PostSignUpObserver {
    ...
    private void signUp(){
        ...
        String alias = presenter.getHandle();
        if(alias.startsWith("@")){
            StringBuilder builder = new StringBuilder(alias);
            builder.deleteCharAt(0);
            alias = builder.toString();
        }
        User newUser = new User(firstName.getText().toString(), lastName.getText().toString(), alias, "");
