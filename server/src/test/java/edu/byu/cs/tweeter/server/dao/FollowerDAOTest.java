...
class FollowerDAOTest {
    ...
    private final Follow follow1 = new Follow(user9, user1);
    ...
    void setup() {
        ...
        Mockito.when(followerDAOSpy.getFollowGenerator()).thenReturn(mockFollowGenerator);
    }
    ...
    @Test
    void testGetFollowers_twoFollowerForUser_limitEqualsUsers(){
    ...
    void testGetFollowers_limitLessThanUsers_endsOnPageBoundary() {
        FollowerRequest request = new FollowerRequest(user1, 2, null);
        FollowerResponse response = followerDAOSpy.getFollowers(request);

        Assertions.assertNotNull(response);
        Assertions.assertEquals(2, response.getFollowers().size());
        Assertions.assertTrue(response.getFollowers().contains(user9));
        Assertions.assertTrue(response.getFollowers().contains(user3));
        Assertions.assertTrue(response.getHasMorePages());


        request = new FollowerRequest(user1, 2, response.getFollowers().get(1));
